...
IMG ?= ghcr.io/b1zzu/kafka-connect-operator:latest
...
KIND_CLUSTER ?= kafka-connect-operator-test-e2e
...
run: manifests generate fmt vet ## Run a controller from your host.
	KCO_PORT_FORWARD_CLIENT=true go run ./cmd/main.go
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name kafka-connect-operator-builder
	$(CONTAINER_TOOL) buildx use kafka-connect-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm kafka-connect-operator-builder
	rm Dockerfile.cross
...
undeploy-e2e: kustomize ## Undeploy the e2e-deployed controller. Call with ignore-not-found=true to ignore resource not found errors during deletion.
	"$(KUSTOMIZE)" build config/e2e-manager | "$(KUBECTL)" delete --ignore-not-found=$(ignore-not-found) --timeout=60s -f -
...
.PHONY: clean-bin
clean-bin: ## Remove downloaded tool binaries (envtest dirs are installed read-only).
