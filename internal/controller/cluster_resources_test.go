...
var _ = Describe("Cluster Resources", func() {
	...
	Describe("deploymentForCluster", func() {
		...
		It("should not have user env vars when envs is empty", func() {
			cluster := newCluster(nil)
			dep := deploymentForCluster(cluster)

			envVars := dep.Spec.Template.Spec.Containers[0].Env
			for _, e := range envVars {
				Expect(*e.Name).NotTo(Equal("MY_VAR"))
			}
		})

		It("should add env vars with plain values", func() {
			cluster := newCluster(nil)
			cluster.Spec.Envs = []corev1.EnvVar{
			...
			envVars := dep.Spec.Template.Spec.Containers[0].Env
			var myVar, anotherVar *corev1ac.EnvVarApplyConfiguration
			for i := range envVars {
				if *envVars[i].Name == "MY_VAR" {
					myVar = &envVars[i]
				}
				if *envVars[i].Name == "ANOTHER_VAR" {
					anotherVar = &envVars[i]
				}
			}
			Expect(myVar).NotTo(BeNil())
			Expect(*myVar.Value).To(Equal("my-value"))
			Expect(anotherVar).NotTo(BeNil())
			Expect(*anotherVar.Value).To(Equal("another-value"))
		})

		It("should add env var with valueFrom secretKeyRef", func() {
			cluster := newCluster(nil)
			cluster.Spec.Envs = []corev1.EnvVar{
				{
					Name: "SECRET_VAR",
			...
			envVars := dep.Spec.Template.Spec.Containers[0].Env
			var secretVar *corev1ac.EnvVarApplyConfiguration
			for i := range envVars {
				if *envVars[i].Name == "SECRET_VAR" {
					secretVar = &envVars[i]
				}
			}
			Expect(secretVar).NotTo(BeNil())
			...
			Expect(*secretVar.ValueFrom.SecretKeyRef.Key).To(Equal("password"))
		})

		It("should add env var with valueFrom configMapKeyRef", func() {
			cluster := newCluster(nil)
			cluster.Spec.Envs = []corev1.EnvVar{
				{
					Name: "CONFIG_VAR",
			...
			envVars := dep.Spec.Template.Spec.Containers[0].Env
			var configVar *corev1ac.EnvVarApplyConfiguration
			for i := range envVars {
				if *envVars[i].Name == "CONFIG_VAR" {
					configVar = &envVars[i]
				}
			}
			Expect(configVar).NotTo(BeNil())
			Expect(configVar.ValueFrom).NotTo(BeNil())
			Expect(configVar.ValueFrom.ConfigMapKeyRef).NotTo(BeNil())
			Expect(*configVar.ValueFrom.ConfigMapKeyRef.Name).To(Equal("my-configmap"))
			Expect(*configVar.ValueFrom.ConfigMapKeyRef.Key).To(Equal("setting"))
		})

		It("should add configmap volumes and mounts with correct paths", func() {
			cluster := newCluster(nil)
			cluster.Spec.ConfigMaps = []kcv1alpha1.ConfigMapMount{
				{Name: "my-config", ConfigMapRef: corev1.LocalObjectReference{Name: "my-config-cm"}},
				{Name: "other-config", ConfigMapRef: corev1.LocalObjectReference{Name: "other-config-cm"}},
			}
			dep := deploymentForCluster(cluster)
		...
		It("should add configmaps together with plugins and secrets", func() {
			cluster := &kcv1alpha1.Cluster{
				Spec: kcv1alpha1.ClusterSpec{
					...
					ConfigMaps: []kcv1alpha1.ConfigMapMount{
						{Name: "my-config", ConfigMapRef: corev1.LocalObjectReference{Name: "my-config-cm"}},
					},
				},
			}
